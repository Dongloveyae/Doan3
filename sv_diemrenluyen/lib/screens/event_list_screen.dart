...
_availableEvents = eventData.where((e) {
  ...
  bool isMultiple = e['score_type'] == 'multiple'; // Check sự kiện tính theo Lượt

  // Nếu là sự kiện tính theo Lượt (multiple), luôn cho phép hiển thị lại để đăng ký lượt mới
  if (isMultiple) {
    return isReg == 0;
  }

  // Ngược lại, nếu tính theo Lần (once), đã checkin thành công thì ẨN ĐI
