...
app.get('/api/mobile/history', (req, res) => {
    ...
    // Sử dụng COLLATE trực tiếp vào các phép so sánh chuỗi để ép đồng bộ bảng mã
    const sql = `
        SELECT 
            e.name, 
            e.points, 
...
// 1. ĐĂNG KÝ SỰ KIỆN (Có cấu hình chặn giới hạn suất theo Khoa/Ngành)
app.post('/api/mobile/register_event', (req, res) => {
    ...
    db.query("SELECT email FROM users WHERE mssv COLLATE utf8mb4_unicode_ci = ? COLLATE utf8mb4_unicode_ci", [mssv], (err, users) => {
        ...
        // Bước 2: Kiểm tra trạng thái và chỉ tiêu ngành của sự kiện
        db.query("SELECT status, faculty_limits, max_participants FROM events WHERE id COLLATE utf8mb4_unicode_ci = ? COLLATE utf8mb4_unicode_ci", [event_id], (err, events) => {
            ...
            if (event.status === 'Ngừng hoạt động' || event.status === 'Đang diễn ra') {
                return res.json({ status: "error", message: "Sự kiện hiện đã đóng hoặc đang diễn ra, không nhận đăng ký thêm." });
            }
